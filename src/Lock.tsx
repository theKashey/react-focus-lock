...
import { ReactFocusLockProps, Shards } from "./interfaces";

const emptyArray: Shards = [];

const FocusLock = React.forwardRef<HTMLDivElement, ReactFocusLockProps>(
  function FocusLockUI(props: ReactFocusLockProps, parentRef) {
    const [realObserved, setObserved] = React.useState<HTMLElement>();
    ...
    const onActivation = React.useCallback(
      ({
        ...
        captureFocusRestore: (el: HTMLElement | null) => HTMLElement;
      }) => {
    ...
    const onDeactivation = React.useCallback(() => {
      ...
      if (onDeactivationCallback) {
        onDeactivationCallback(observed.current as HTMLElement);
      }
      update((s) => s + 1);
        ...
        if (focusRestore) {
          ...
          if (howToReturnFocus) {
            const returnFocusOptions =
              typeof howToReturnFocus === "object"
                ? howToReturnFocus
                : undefined;
    ...
    const setObserveNode = React.useCallback((newObserved: HTMLElement) => {
    ...
    const mergedRef = useMergeRefs([parentRef, setObserveNode as any]); // TODO verify this call.
...
FocusLock.propTypes = {
  ...
  as: oneOfType([string, func, object]) as any,
  lockProps: object,
